...
export default function MediaPanel({ input }: Props) {
  ...
  if (!input.trim()) {
    return (
      <div className="mx-auto flex aspect-video h-full max-h-full items-center justify-center rounded-xl border border-border bg-surface p-6 text-center text-hud-sub text-hud-muted">
  ...
  if (!src) {
    ...
    return (
      <div className="mx-auto flex aspect-video h-full max-h-full flex-col items-center justify-center gap-2 rounded-xl border border-red-800 bg-red-950/40 p-6 text-center">
  ...
  return (
    // ALWAYS height-driven: 16:9 derived from the box the caller gives us, centred horizontally.
    //
    // Width-driven sizing is what caused the overlap bug — a 16:9 box derived from a 62%-wide
    // column is ~500px tall, which the column's remaining height could not absorb, so the content
    // overflowed its flex row and the liveness strip drew on top of the graph. Callers now hand
    // this a BOUNDED-height box and the width follows, which cannot overflow.
    //
    // Never stretched to fill an arbitrary box either: filling a wider-than-16:9 container makes
    // YouTube pillarbox INSIDE the iframe, putting black bars inside our own border.

...
import { calculateErgProgress } from '../../src/services/workoutService'
...
describe('ERG Mode Logic', () => {
  ...
  // Wrapper: pass mockTimer.getCurrentTime() as 'now' to the pure service function
  function ergProgress(stepStartTime, durationMinutes) {
    return calculateErgProgress(stepStartTime, durationMinutes, mockTimer.getCurrentTime())
  }
  ...
  describe('ERG Progress Bar Calculation', () => {
    test('progress should be 0% at step start', () => {
      const W = initializeWorkout([{ type: 'erg', duration: 5, power: 200 }])

      const progress = ergProgress(W.stepStartTime, 5)
    ...
    test('progress should be 50% at half duration', () => {
      ...
      const progress = ergProgress(W.stepStartTime, 10)
    ...
    test('FIX CASE: stepStartTime reset on workout start prevents incorrect progress', () => {
      ...
      const progress = ergProgress(newW.stepStartTime, 5)
  ...
  describe('ERG Step Auto-Advance', () => {
    ...
    test('timeout duration should match progress bar duration', () => {
      ...
      initializeWorkout([{ type: 'erg', duration: durationMinutes, power: 200 }])
      ...
      const progress = ergProgress(global.Hybrid.state.workout.stepStartTime, durationMinutes)
      expect(progress).toBe(100)
    })
    ...
    test('progress and timeout should be synchronized', () => {
      ...
      for (const fraction of checkpoints) {
        ...
        const progress = ergProgress(W.stepStartTime, durationMinutes)
  ...
  describe('ERG Step Distance Calculation', () => {
    test('distance should accumulate based on speed and time', () => {
      initializeWorkout([{ type: 'erg', duration: 5, power: 200 }])
      ...
      const stepElapsedSec =
        (mockTimer.getCurrentTime() - global.Hybrid.state.workout.stepStartTime) / 1000
  ...
  describe('Edge Cases', () => {
    test('zero duration step should be handled gracefully', () => {
      initializeWorkout([{ type: 'erg', duration: 0, power: 200 }])

      // Progress with zero duration results in division by zero (NaN)
      // The actual app should guard against this case
      const progress = ergProgress(global.Hybrid.state.workout.stepStartTime, 0)
    ...
    test('very short duration should work correctly', () => {
      ...
      const W = initializeWorkout([{ type: 'erg', duration: durationMinutes, power: 200 }])

      startErgStep(0)

      // At 15 seconds (50%)
      mockTimer.advanceTime(15 * 1000)
      let progress = ergProgress(W.stepStartTime, durationMinutes)
      expect(progress).toBeCloseTo(50, 1)

      // At 30 seconds (100%)
      mockTimer.advanceTime(15 * 1000)
      progress = ergProgress(W.stepStartTime, durationMinutes)
      expect(progress).toBe(100)
    })

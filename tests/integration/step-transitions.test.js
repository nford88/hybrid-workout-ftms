...
describe('Step Transitions Integration', () => {
  ...
  describe('SIM → ERG Transitions', () => {
    test('properly cleans up SIM mode before starting ERG', () => {
      ...
      const ergZeroCall = ftmsLog.find(
        (call) => call.method === 'setErgWatts' && call.args[0] === 0
      )
      ...
      const ergPowerCall = ftmsLog.find(
        (call) => call.method === 'setErgWatts' && call.args[0] === 200
      )
    ...
    test('handles rapid SIM→ERG transitions without conflicts', () => {
      ...
      const simCalls = result.ftmsLog.filter(
        (call) => call.method === 'setSim' || call.method === 'rampSim'
      )
    ...
    test('prevents ERG commands from being ignored after SIM', () => {
      ...
      const finalErgCall = result.ftmsLog.filter((call) => call.method === 'setErgWatts').pop() // Get last ERG call
  ...
  describe('Multiple Step Transitions', () => {
    test('handles complex workout with multiple transitions', () => {
      ...
      allTransitions.forEach((transition, _index) => {
        ...
        const hasSimCommand = transition.ftmsLog.some(
          (call) => call.method === 'setSim' || call.method === 'rampSim'
        )
    ...
    test('maintains correct state between rapid transitions', () => {
      ...
      simulateStepTransition('erg', 'sim')

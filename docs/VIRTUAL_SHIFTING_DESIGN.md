...
| L22 | The KICKR honours the FTMS `Crr` byte (0x11), not only the grade | **PARTIALLY CONFIRMED 2026-08-05** (`virtual-shifting/experiments/17-crr-cw-sweep-protocol.md` §10): speed-matched within block, Crr 0.004→0.020 raised power in **every** bin — mean **+74 W vs +88 W predicted (84%)**. **`Cw` NOT established** (5-47% of prediction, below the scatter). Two corrections this produced: predictions about trainer-side behaviour must use the **trainer's own speed** (5.1-6.8 m/s), not `v_virt` (7.13) — that alone moved the result from 53% to 84%; and absolute power is still not predicted by the model (MAE ~120 W, negative R², ~63 W baseline over-prediction), so only differences-in-differences are sound. Needs replication — one lap per condition cannot separate the trainer honouring the byte from the rider pushing harder |
